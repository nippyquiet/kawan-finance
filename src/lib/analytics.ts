...
export async function getAnalytics(pocketId: number | null): Promise<Analytics> {
  ...
  for (const tx of allRecentTx) {
    ...
    if (tx.type === "INCOME") byMonth[m].income += tx.amount;
      ...
      if (tx.categoryId) catSpending[tx.categoryId] = (catSpending[tx.categoryId] || 0) + tx.amount;
    }
  }
